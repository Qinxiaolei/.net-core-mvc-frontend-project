...
export const S_METHOD = {
    ...
    renewUIAfterGuestLogin: function (res) {
        let virDom = gMEMO.oVirDom;
        let login = virDom.cmpnReg.login;
        //取消按钮旋转、禁点状态
    ...
    renewUIAfterUserLogin: function (res) {
        ...
        if (res.sExt.length > 0) {
            let oExt = JSON.parse(res.sExt);
    ...
    initByLastLogin: function () {
        ...
        if (localLofinInfoSet.length > 0) {
            ...
            //ajax登录
            S_AJAX.userLogin(function (res) {
                S_METHOD.renewUIAfterUserLogin(res); //刷新UI
            }, function (cError) {
